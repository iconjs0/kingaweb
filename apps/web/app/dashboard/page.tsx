  ...
  return (
    <main className="dashboardPage">
      ...
      <section className="dashboardMain" id="overview">
        ...
        <section className="assetSection" id="assets"><div className="sectionHeading"><div><span className="eyebrow">Attack surface</span><h2>Your assets</h2></div><button className="button buttonSmall" type="button" disabled>Add asset — next</button></div>
          {assets.length ? <div className="assetList">{assets.map((asset) => <article key={asset.id}><span className="assetMark">W</span><div><b>{asset.hostname}</b><small>{asset.status.replaceAll("_", " ")}</small></div><span className="assetStatus">Awaiting verification</span></article>)}</div> : <div className="emptyState"><span>◎</span><h3>No assets registered yet</h3><p>The next vertical slice will connect domain onboarding to the secure DNS verification contract already available in the API.</p></div>}
        </section>
      </section>
    </main>
  );
